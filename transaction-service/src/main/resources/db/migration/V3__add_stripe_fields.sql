...
ADD COLUMN stripe_transfer_status VARCHAR(50),
...
CREATE INDEX idx_stripe_transfer_id ON transactions(stripe_transfer_id);
...
COMMENT ON COLUMN transactions.stripe_transfer_id IS 'Stripe transfer ID returned from Stripe API';
COMMENT ON COLUMN transactions.stripe_transfer_status IS 'Stripe transfer status: pending, in_transit, paid, failed, canceled';
COMMENT ON COLUMN transactions.stripe_failure_code IS 'Stripe failure code if transfer failed';
COMMENT ON COLUMN transactions.stripe_failure_message IS 'Stripe failure message if transfer failed';
